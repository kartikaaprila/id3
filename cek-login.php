...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['id3_id'] = $rows['id_user']; // mengisi session
        $_SESSION['id3_nama'] = $rows['nama'];
        $_SESSION['id3_username'] = $rows['username'];
        $_SESSION['id3_level'] = $rows['level'];
        $_SESSION['id3_id_siswa'] = get_id_siswa_by_id_user($db, $rows['id_user']);

        $level_name = ($_SESSION['id3_level']==1)?"Bendahara":"Ketua";
        $_SESSION['id3_level_name'] = $level_name;
        $_SESSION['id3_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['id3_last_login'] = date("d-m-Y H:i:s");
